...
bool Program::init()
{
	...
	// Creates Shader objects of time GL_VERTEX_SHADER or GL_FRAGMENT_SHADER (they are of type enum)
	GLuint VS = glCreateShader(GL_VERTEX_SHADER);
	GLuint FS = glCreateShader(GL_FRAGMENT_SHADER);

	// Read shader sources
	// vShaderString is maybe name of shader file
	std::string vShaderString = readFileAsString(vShaderName);
	std::string fShaderString = readFileAsString(fShaderName);
	// converts std::string to c string (null terminated char pointer)
	const char *vshader = vShaderString.c_str();
	const char *fshader = fShaderString.c_str();
	// glShaderSource replaces source code in shader with third arg
	// CHECKED_GL_CALL GLSL macro which checks for errors
...
// uninstall a program object as part of current rendering state
void Program::unbind()
{
...
// adds a uniform variable to uniform hash table
